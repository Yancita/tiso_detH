# -*- coding: utf-8 -*-
"""
Created on Wed Oct  8 19:02:20 2025
...
Sxy2 = Sx@Sx - Sy@Sy   # in-plane projection
...
def monte_carlo_avg(b_ratio, Dz=1.0, Exy=0.1, N=3000):
    """
    Isotropic average of <Sz^2> and <Sx^2+Sy^2> 
    for ground state of Hamiltonian with |B|/Dz = b_ratio.
    """
    Bmag = b_ratio * Dz
    nvecs = random_unit_vectors(N)

    vals_z, vals_xy = [], []
    for nx, ny, nz in nvecs:
        Bx, By, Bz = Bmag*nx, Bmag*ny, Bmag*nz
        H = Dz*(Sz2 - (2/3)*I3) + Exy*(Sx@Sx - Sy@Sy) + Bx*Sx + By*Sy + Bz*Sz
        eigvals, eigvecs = np.linalg.eigh(H)
        psi0 = eigvecs[:, np.argmin(eigvals)]  # ground state
        vals_z.append(np.vdot(psi0, Sz2 @ psi0).real)
        vals_xy.append(np.vdot(psi0, Sxy2 @ psi0).real)

    return np.mean(vals_z), np.mean(vals_xy)

def pade_appro(n, b_ratio, Dz, Exy):
    if n == 0: # Phiz _ 0
        a1 = 1 + 3 * (Exy/Dz)**2 
        a2 = 0.24 + 1.6 * (Exy/Dz)**2 
        a3 = 0.6 + 2.2 * (Exy/Dz)**2 
        return  2/3 - 2/3/(1 + a1 * b_ratio**2 /(1 + a2 * b_ratio**2/(1+a3 *b_ratio**1)))

    if n == 1: # Phiz _ 2
        a1 = (Dz/(Exy + Dz))**2
        a2 = 4/5 * (Dz/(Exy + Dz))**2
        a3 = 1
        return  1/(1 + a1 * b_ratio**2 /(1 + a2 * b_ratio**2/(1+a3 *b_ratio**1)))

    if n == 2: # Phixy _ 2
        a1 = 1 + 1/(6 * (Exy * Dz)**2 )
        a2 = -1.23 + 0.28 /(Exy * Dz)**2
        a3 = -13.5 *(Exy/Dz) + 2.1 / (Exy * Dz)
        return  1/(1 + a1 * b_ratio**2 /(1 + a2 * b_ratio**2/(1+a3 *b_ratio**1)))


# Sweep B/Dz
Dz, Exy = 1.0, 0.05
bs = np.linspace(0, 5, 30)
mc_z, mc_xy = [], []
mc_zDn, mc_xyDn = [], []
theo_z, theo_xy = [], [] 
pade_z = []
...
for b in bs:
    #vz, vxy = monte_carlo_avg(b, Dz, Exy, N=5000)
    #mc_z.append(vz)
    #mc_xy.append(vxy * 10)
    #vzDn, vxyDn = monte_carlo_avg(b, -Dz, Exy, N=5000)
    #mc_zDn.append(vzDn)
    #mc_xyDn.append(vxyDn)
    
    theo_z.append( tisoZ(0, b, Dz, Exy) + 2./3. )
    theo_xy.append( tisoXY(2, b, Dz, Exy))
    
    pade_z.append( pade_appro(0, b, Dz, Exy) )
    pade_xy.append( pade_appro(2, b, Dz, Exy) )
    
# Plot
plt.figure(figsize=(7,5))
#plt.plot(bs, mc_z, '>-', label=r"$\langle S_z^2 \rangle$, D > 0")
#plt.plot(bs, mc_xy,  '>-', label=r"$\langle S_x^2-S_y^2 \rangle$")

plt.plot(bs, theo_z,  '-.', label=r"$\langle S_z^2 \rangle$_thoe")
plt.plot(bs, theo_xy,  '-.', label=r"$\langle S_x^2-S_y^2 \rangle$_thoe")

plt.plot(bs, pade_z,  '-.', label=r"$\langle S_z^2 \rangle$_pade")
plt.plot(bs, pade_xy,  '-.', label=r"$\langle S_x^2-S_y^2 \rangle$_pade")

plt.xlabel(r"$B/D_z$")
plt.ylabel("Isotropic average (ground state)")
plt.title("Triplet exciton: isotropic average projections")
plt.legend()
